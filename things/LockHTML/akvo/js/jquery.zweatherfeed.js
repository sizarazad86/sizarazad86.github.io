...
(function($){
	...
	$.fn.weatherfeed = function(locations, options, fn) {	
		...
		akvo has no css to nanage more information than displayed using this configuration. */
		var defaults = {
			unit: 'c',
			image: true,
			country: false,
		...
		return this.each(function(i, e) {
			...
			var _process = function(e, feed, options) {
				...
				if (feed.description != 'Yahoo! Weather Error') {
					...
					// Add weather data
					var html = '<div class="weatherCity">'+ feed.location.city +'</div>';
					html += '<div class="icon_temp"><div class="weatherItem '+ row +' '+ daynight +'"';
					if (options.image) {
						html += ' style="background-image: url(./Images/icons-'+((akvoOptions.black_mode)?'black/':'white/')+ feed.item.condition.code + daynight.substring(0,1) +'.png); background-repeat: no-repeat;"';
					}
					html += '></div>';
					...
					var html = '<div class="weatherCity">Unknown</div>' +
								'<div class="icon_temp">' +
									'<div class="weatherItem odd night" '+
									'style="background-image: url(./Images/icons-'+((akvoOptions.black_mode)?'black':'white')+'/3200.png); background-repeat: no-repeat;">' +
									'</div><div class="weatherTemp">0°' +
								'</div></div>' +
								'<div class="weatherDesc">Weather';
				}
